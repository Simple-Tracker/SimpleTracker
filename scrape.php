...
if (isset($_SERVER['QUERY_STRING'])) {
	...
	$clientInfoHashList = array_map(function ($v) { $v = strtolower(bin2hex($v)); if (strlen($v) !== 40) { die(GenerateBencode(array('failure reason' => ErrorMessage[3]))); } return $v; }, $info_hash_match[1]);
}
...
if (DBPort === null) {
	die(GenerateBencode(array('failure reason' => ErrorMessage[11])));
} else {
